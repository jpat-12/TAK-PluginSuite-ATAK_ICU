...
Target: **ATAK-CIV 5.7.0** SDK. Modeled on the QuickCapture plugin build.

1. Copy `local.properties.example` → `local.properties` and set your paths
   (`sdk.dir`, `sdk.path` = the ATAK-CIV-5.7.0-SDK folder, `takdev.plugin`).
   The SDK signing keystore is auto-staged from `sdk.path` at build time.
2. Ensure `app/libs/main.jar` is the 5.7.0 SDK `main.jar` (gitignored; copy from the SDK).
3. Build:
