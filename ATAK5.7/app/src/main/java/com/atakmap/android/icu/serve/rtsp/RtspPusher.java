...
public class RtspPusher {
    ...
    // ── Optional third track: MISB ST 0601 KLV (smpte336m), mirroring RtspServer ──
    private boolean hasKlv;
    private int     klvTrackId = -1;                     // assigned in SDP order
    private int     klvRtpChannel = 4;                   // interleaved channel from KLV SETUP
    private int     klvSeq;
    private final int klvSsrc = new Random().nextInt();

    private static final int PT_VIDEO = 96, PT_AUDIO = 97, PT_KLV = 98;
    ...
    /** Announce a MISB ST 0601 KLV metadata track alongside the video. Like
     *  {@link #setAudio}, must be called before {@link #publish} — the SDP is built once
     *  during the handshake and the track id depends on m-line order. */
    public void setKlv(boolean enabled) { this.hasKlv = enabled; }
    ...
    public void publish(byte[] sps, byte[] pps) throws IOException {
        ...
        if (hasKlv) {
            // Metadata track on the next free interleaved pair. Non-fatal like audio: a
            // server that won't take an application/smpte336m track should still get video.
            int ch = hasAudio ? 4 : 2;
            Resp kSetup = request("SETUP", baseUrl + "/trackID=" + klvTrackId,
                    "Transport: RTP/AVP/TCP;unicast;interleaved=" + ch + "-" + (ch + 1)
                            + ";mode=record\r\n", null);
            Log.d(TAG, "SETUP klv -> " + kSetup.code + " transport=" + transportLine(kSetup.headers));
            if (kSetup.code != 200) {
                logFail("SETUP klv", kSetup);
                hasKlv = false;
            } else {
                klvRtpChannel = parseRtpChannel(kSetup.headers);
                if (klvRtpChannel == 0) klvRtpChannel = ch;      // default if server didn't echo
            }
        }
    ...
    /** One MISB ST 0601 KLV packet on the metadata track. Same 90 kHz clock as video, its
     *  own SSRC/sequence. ST 0601 sets sit well under the interleaved frame limit, so a
     *  packet always fits in one RTP message — no fragmentation, unlike large H.264 NALs. */
    public void sendKlv(byte[] klv, long ptsUs) {
        if (out == null || !hasKlv || klv == null || klv.length == 0) return;
        long ts = ptsUs * 90L / 1000L;
        try {
            writeInterleaved(klvRtpChannel,
                    buildRtp(klv, 0, klv.length, true, ts, PT_KLV, klvSeq++, klvSsrc));
        } catch (IOException e) {
            if (!sendErrLogged) {
                sendErrLogged = true;
                Log.w(TAG, "sendKlv failed (" + e.getMessage() + ")");
            }
        }
    }
    ...
    private String buildSdp() {
        ...
        if (hasKlv) {
            // Track id follows m-line order, so KLV is 2 with audio present and 1 without —
            // it must not be hardcoded or the SETUP URL won't match what we announced.
            klvTrackId = hasAudio ? 2 : 1;
            sdp.append("m=application 0 RTP/AVP ").append(PT_KLV).append("\r\n")
               .append("a=rtpmap:").append(PT_KLV).append(" smpte336m/90000\r\n")
               .append("a=control:trackID=").append(klvTrackId).append("\r\n");
        }
        return sdp.toString();
    }
