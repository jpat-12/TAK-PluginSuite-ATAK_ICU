...
public class H264Encoder {
    ...
    private volatile int     outW, outH;  // encoded frame size (see start())

    /** Width of the encoded frame — the capture width, swapped for portrait orientations.
     *  0 until {@link #start}. Needed by anything that must declare the stream's real
     *  dimensions, e.g. the MP4 track {@link Mp4Recorder} writes. */
    public int getOutputWidth()  { return outW; }
    public int getOutputHeight() { return outH; }
    ...
    public Surface start(EncoderConfig config, Callback callback) {
        try {
            ...
            outW = swap ? config.captureH : config.captureW;
            outH = swap ? config.captureW : config.captureH;
